...
contract PullSplit is SplitWalletV2 {
    ...
    function distribute(
        ...
        if (_performWarehouseTransfer) {
            uint256 amount = (
                _token == NATIVE_TOKEN ? address(this).balance : IERC20(_token).balanceOf(address(this))
            ) - 1;
    ...
    function depositToWarehouse(address _token, uint256 _amount) public {
        if (_token == NATIVE_TOKEN) {
            ...
            try SPLITS_WAREHOUSE.deposit({ receiver: address(this), token: _token, amount: _amount }) {} catch {
                IERC20(_token).approve({ spender: address(SPLITS_WAREHOUSE), value: type(uint256).max });
