...
import { deploymentConfig, MAX_DISTRIBUTION_FEE, PERCENTAGE_SCALE } from "../constants";
...
task("deploy:split")
  ...
  .setAction(async function (taskArguments: TaskArguments, { ethers, getChainId, getNamedAccounts, network }) {
    ...
    if (total != PERCENTAGE_SCALE.toNumber()) {
      throw new Error(`percentAllocations mismatch 0xSplit PERCENTAGE_SCALE (${total} != ${PERCENTAGE_SCALE.toString()})`);
    }
    ...
    const splitMain = (await ethers.getContractAt('ISplitMain', deploymentConfig[chainId]?.splitMain, signer)) as ISplitMain;
    ...
    const splitAddress = 
      receipt.events?.[0].topics[1] && ethers.utils.hexStripZeros(receipt.events?.[0].topics[1]);

    console.log(`New Split contract deployed to ${network.name}:`, splitAddress, ` txhash: (${receipt.transactionHash})`);
  });
