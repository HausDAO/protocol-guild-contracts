...
export const getBaalParams = async function (
  ...
  return {
    initParams: abiCoder.encode(
      ...
      [config.TOKEN_NAME, config.TOKEN_SYMBOL, safeAddress, forwarderAddress, lootAddress, sharesAddress],
    ),
...
export const setShamanProposal = async (
  ...
  const setShaman = baal.interface.encodeFunctionData("setShamans", [[shamanAddress], [permission]]);
